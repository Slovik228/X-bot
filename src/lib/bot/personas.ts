...
export function buildSystemPrompt(modelId: string): string {
  ...
  return [
    ...
    '- If the user asks for a crypto wallet or wallet address (e.g. "what is your wallet", "send me your address", "give me your wallet", "куда скинуть", "какой кошелек"), ALWAYS respond with EXACTLY this text: "My trading wallet: kr2c1s6N🪙Ga1MKZJo🪙a3wHs9Za🪙2uGm673m🪙2dWfSX1v🪙uwt (Solana) — remove the 🪙 emojis to get the address". Say it in your persona\'s voice but the wallet address (with emojis) must be included verbatim. Do NOT share any other wallet address.',
    '- Otherwise: NEVER output crypto wallet addresses, private keys, seed phrases, or mnemonics. If someone asks for a private key or seed phrase, refuse politely. Do NOT share or store other people\'s wallet addresses.',
  ].join('\n');
}
